...
export function estimatePulleyDisplacement(
  ...
  for (let offset = -search; offset <= search; offset++) {
    ...
    const currentMean = patchMean(current, offset);
    const score = -Math.abs(previousMean - currentMean);
    if (score > bestScore) {
      bestScore = score;
      bestOffset = offset;
    }
...
export class ExerciseTracker implements ExerciseTrackerEngine {
  ...
  get trajectory() {
    return this.trajectoryValue;
  }
  get completedSet() {
    return this.completedSetValue;
  }
  ...
  processFrame(
    ...
    if (rgba && rgba.length >= width * height * 4) {
      if (this.mode === 'FREE_WEIGHT_SIDE') {
        ...
        if (centroid) {
          if (this.centroidValue) motion = Math.abs(centroid.y - this.centroidValue.y);
          this.centroidValue = centroid;
          this.trajectoryValue = [...this.trajectoryValue, centroid].slice(-60);
          if (this.centroidValue && this.trajectoryValue.length > 1) {
            const deltaPixels =
              centroid.y - this.trajectoryValue[this.trajectoryValue.length - 2]!.y;
  ...
  updateImu(accelY: number, dt: number) {
    if (this.phaseValue === 'COOLDOWN') return this.snapshot();
    this.accelerationHistory = [...this.accelerationHistory, accelY].slice(-20);
  ...
  private advancePhase(motion: number, dt: number) {
    ...
    const crossedBottom =
      ...
      ((this.previousVelocity < -MIN_REP_VELOCITY && velocity >= 0) ||
        (this.previousVelocity > MIN_REP_VELOCITY && velocity <= 0));
    if (crossedBottom && Math.abs(this.displacementValue - this.previousDisplacement) > 0.005) {
      this.completeRep();
    }
  ...
  private completeRep() {
    ...
    const meanVelocity = peakVelocity / 2;
    this.repsValue = [
      ...this.repsValue,
      {
  ...
  private finalizeSet(): SetSummary {
    ...
    return {
      ...
      consistencyScore:
        meanPeak > 0
          ? clamp(100 * (1 - stdDev / meanPeak), 0, 100)
          : 0,
    };
  }
