...
function Feedback({ result, fallback, error, colors, onRecovery, onRestart }: { result: SetAnalysisResult | null; fallback: SetSummary; error: string | null; colors: Colors; onRecovery: () => void; onRestart: () => void }) {
  const r = result;
  const mean = r?.mean_velocity_ms ?? 0;
...
export default function MotionTrackerScreen() {
  ...
  const finish = useCallback(async (tracked?: SetSummary) => { if (finished.current) return; finished.current = true; const current = tracked ?? tracker.stopSet().completedSet ?? summary; const source = demoMode ? 'DEMO' : 'IMU'; const decorated = { ...current, durationSec: (Date.now() - startedAt.current) / 1000, sampleCount: samples.current.length, dataQuality: source === 'DEMO' ? 'DEMO' : 'GOOD', confidence: source === 'DEMO' ? 'LOW' : 'MEDIUM', inferenceSource: source, limitations: source === 'DEMO' ? ['Seeded rehearsal data; not accuracy validation.'] : ['IMU integration is an estimate; keep placement stable.'] } as SetSummary; setSummary(decorated); setPhase('PROCESSING'); setError(null); try { const response = await analyzeSet({ exercise_name: 'Squat', weight_kg: Number(weight), target_reps: Number(targetReps), total_sets: 1, phone_placement: placement === 'barbell' ? 'barbell' : 'pocket', samples: source === 'DEMO' ? seededSamples() : samples.current }); setResult(response); setPhase('FEEDBACK'); } catch (e) { setError(e instanceof Error ? e.message : 'Analysis request failed'); setPhase('FEEDBACK'); } }, [demoMode, placement, summary, targetReps, tracker.stopSet, weight]);
  ...
  if (phase === 'FEEDBACK') return <View style={[styles.root, { paddingTop: insets.top }]}><Feedback result={result} fallback={summary} error={error} colors={colors} onRecovery={() => setPhase('RECOVERY')} onRestart={reset} /></View>;
